...
import com.kacper.and_krakgo.sign_in.register_part2.RegisterPart2Fragment;
...
public class RegisterFragment extends Fragment implements RegisterContract.View {
    ...
    private void goToNextPage() {
        if (!(mEmailInput.getEditText().length() > 0 && mPasswordInput.getEditText().length() > 0 && mRetypePasswordInput.getEditText().length() >0) ||
            ...
            ToastMessageHelper.showShortToast(getContext(), R.string.you_have_to_be_above_18);
        } else if (!mtermsAcceptedCheckbox.isChecked()) {
            ToastMessageHelper.showShortToast(getContext(), R.string.you_have_to_accept_terms);
    ...
    public void showError(String error) {
        ToastMessageHelper.showShortToast(getContext(), error);
